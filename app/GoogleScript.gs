/**
 * Galaxy 2K26 Registration Script (UPDATED - EMAIL V4 + PER-EVENT SHEETS + PROJECT/PAPER FIELDS + FOOD/ACCOMMODATION)
 * 
...
function doPost(e) {
    ...
    try {
        ...
        if (!mainSheet) {
            ...
            // Added new columns to headers: Project/Paper fields + Food + Accommodation
            mainSheet.appendRow([
        ...
        if (teamDetails) {
            for (const [evt, info] of Object.entries(teamDetails)) {
                if (events.includes(evt) && (info.leadName || (info.members && info.members.length))) {
                    ...
                    // Capture specialized fields
                    if (evt.toUpperCase() === 'PROJECT WAR') {
                        if (info.projectTitle) {
                            teamDetailsString += `   > Title: ${info.projectTitle}, Domain: ${info.projectDomain}\n`;
                            projectTitle = info.projectTitle; 
                            projectDomain = info.projectDomain; 
                        }
                    }
                    if (evt.toUpperCase() === 'PAPER PRESENTATION') {
                        if (info.paperTitle) {
                            teamDetailsString += `   > Paper: ${info.paperTitle}\n`;
        ...
        if (Array.isArray(events)) {
            events.forEach(eventName => {
                ...
                addToCategorySheet(ss, eventName.toUpperCase(), eventName, row, teamDetails?.[eventName], extras);
            });
        }
...
function addToCategorySheet(ss, sheetName, eventName, mainRowData, teamInfo, extras) {
    let sheet = ss.getSheetByName(sheetName);
    if (!sheet) {
        sheet = ss.insertSheet(sheetName);
        // Added new columns to headers here as well
        sheet.appendRow([
            "Timestamp", "Full Name", "Email", "Phone", "College", "Department", "Year", 
    ...
    if (teamInfo) {
        ...
        // Specific logic: Only add Title/Domain if this ROW is for the relevant event
        if (eventName.toUpperCase() === 'PROJECT WAR') {
            pTitle = teamInfo.projectTitle || (extras ? extras.projectTitle : "");
            pDomain = teamInfo.projectDomain || (extras ? extras.projectDomain : "");
        }
        if (eventName.toUpperCase() === 'PAPER PRESENTATION') {
            pptTitle = teamInfo.paperTitle || (extras ? extras.paperTitle : "");
        }
    }

    // mainRowData indices:
    // 0:Time, 1:Name, 2:Email, 3:Phone, 4:College, 5:Dept, 6:Year 
    // ... 9:TxID, 10:Link ... 
    // 14: Food, 15: Accommodation (Indices from mainSheet row construction above)
    // Wait, let's verify mainRowData indices from Step 4 above.
    // row = [time, name, email, phone, college, dept, year, events, team, tx, link, pTitle, pDomain, paperTitle, food, accom]
...
function sendEmail(name, email, events, teamDetails) {
    ...
    <body style="margin: 0; padding: 0; background-color: #000000; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
      ...
      <div style="max-width: 600px; margin: 20px auto; background: #0a0a0a; border-radius: 16px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.7); border: 1px solid #333;">
        ...
        <div style="padding: 40px 30px; background: linear-gradient(180deg, #0a0a0a 0%, #111111 100%);">
          ...
          <div style="background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.1); padding: 20px; border-radius: 12px; margin-top: 30px; margin-bottom: 30px;">
            <h3 style="color: #ff4d4d; margin: 0 0 15px 0; font-size: 16px; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 10px;">📋 Common Rules</h3>
            <ul style="color: #ccc; font-size: 13px; line-height: 1.6; padding-left: 20px; margin: 0;">
              <li style="margin-bottom: 8px;"><strong style="color: #fff;">ID Card:</strong> Mandatory for all participants.</li>
              <li style="margin-bottom: 8px;"><strong style="color: #fff;">Dress Code:</strong> Formal / Professional attire is required.</li>
              <li><strong style="color: #fff;">Reporting:</strong> 8:50 AM at <span style="color: #4285f4;">Kalaiarangam (Auditorium), GCE Erode</span>.</li>
            </ul>
          </div>
  ...
  `;
    ...
    // --- ATTACHMENT LOGIC ADDED HERE ---
    var attachments = [];
    try {
        var brochureId = "16zx0RVTrW_GnUB8OFaVII8Eqoa5NtJ5u"; // ID Extracted from user link
        var brochure = DriveApp.getFileById(brochureId);
        attachments.push(brochure.getAs(MimeType.PDF));
    } catch(e) {
        Logger.log("Error attaching brochure: " + e.toString());
    }
    // ------------------------------------
